...
#define DB_MSG_HEADER_SIZE 8
#define DB_HELLO_HEADER_SIZE 28
#define DB_APPOINTMENT_HEADER_SIZE 8
#define DB_ACKHELLO_HEADER_SIZE 44
#define DB_DONTFORWARD_HEADER_SIZE 8
#define DB_DONTFORWARD_TUPLE_SIZE 4
#define DB_LC2LC_HEADER_SIZE 8
#define DB_LC2LC_TUPLE_SIZE 4
...
MessageHeader::Deserialize (Buffer::Iterator start)
{
  ...
  switch (m_messageType)
    ...
    case ACKHELLO_MESSAGE:
      size +=
        m_message.ackhello.Deserialize (i, m_messageSize - DB_MSG_HEADER_SIZE);
      break;
    case DONT_FORWARD:
      size +=
        m_message.dontforward.Deserialize (i, m_messageSize - DB_MSG_HEADER_SIZE);
      break;
    case LC2LC:
      size +=
        m_message.lc2lc.Deserialize (i, m_messageSize - DB_MSG_HEADER_SIZE);
      break;
...
{
  return (DB_ACKHELLO_HEADER_SIZE);
}
  ...
  uint32_t messageSize)
{
  ...
  NS_ASSERT (messageSize == DB_ACKHELLO_HEADER_SIZE);
...
{
  return (DB_DONTFORWARD_HEADER_SIZE +
    this->list.size () * DB_DONTFORWARD_TUPLE_SIZE);
}
  ...
  uint32_t messageSize)
{
  ...
  this->list.clear ();
  NS_ASSERT (messageSize >= DB_DONTFORWARD_HEADER_SIZE);
  ...
  NS_ASSERT ((messageSize - DB_DONTFORWARD_HEADER_SIZE) %
    (DB_DONTFORWARD_TUPLE_SIZE) == 0);

  uint32_t numTuples = (messageSize - DB_DONTFORWARD_HEADER_SIZE)
    / (DB_DONTFORWARD_TUPLE_SIZE);
...
{
  return (DB_LC2LC_HEADER_SIZE +
    this->list.size () * DB_LC2LC_TUPLE_SIZE);
}
  ...
  uint32_t messageSize)
{
  ...
  this->list.clear ();
  NS_ASSERT (messageSize >= DB_LC2LC_HEADER_SIZE);
  ...
  uint32_t numTuples = (messageSize - DB_LC2LC_HEADER_SIZE)
    / (DB_LC2LC_TUPLE_SIZE);
  NS_ASSERT ( numTuples == this->list_size );
